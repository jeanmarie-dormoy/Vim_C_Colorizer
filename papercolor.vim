...
let s:themes['default'].dark = {
      ...
      \       'diffchange_bg': ['#005f5f', '23'],
		\		'orange': ['#d78700', '172'], 
		\		'bleuvert': ['5fd787', '78'], 
		\		'jaunevert' : ['#d7ff00', '190']
      \     }
      \   }
" orange bleuvert et jaunevert conforme
" }}}
...
""--------COLORS JM --------------------------------------------
	let s:bleuvert	= get(s:palette, 'bleuvert' , get(s:palette, 'color09')) + ['LightRed']
	let s:jaunevert	= get(s:palette, 'jaunevert' , get(s:palette, 'color11')) + ['LightYellow']
	let s:orange	= get(s:palette, 'orange' , get(s:palette, 'color11')) + ['LightYellow']
...
fun! s:set_highlightings_variable()
  ...
  "JM try vimregrep default s:search_fg, s:search_bg
...
"-----------------------------------------------------------------"
	...
	"			TYPE
	"-------------------------------------------------------"	
	"call s:HL("cType", s:pink, "", "")
	highlight cType ctermfg=42
	highlight mycType ctermfg=42
	"highlight mycType ctermbg=darkmagenta ctermfg=black
	highlight mycTypeFunctionPointer ctermfg=42
	highlight cFuncReturnFuncArgs ctermfg=42
	"highlight mycTypeFunctionPointer ctermbg=darkmagenta ctermfg=black
	
	call s:HL("cBoolean", s:green, "", "")
	call s:HL("cCharacter", s:olive, "", "")
	"s:pink
	call s:HL("cppModifier", s:pink, "", "")
	
	"NULL constant
	"call s:HL("cConstant", s:bleuvert, "", "")
	highlight cConstant ctermfg=80
	...
	"highlight cTypeDefStruct ctermbg=darkblue ctermfg=white
	"highlight cStructOrUnionStart ctermbg=darkgreen ctermfg=black
	highlight cStructOrUnionStart ctermfg=42
	"highlight cStructOrUnionEnd ctermbg=darkgreen ctermfg=black
	highlight cStructOrUnionEnd ctermfg=42
	"highlight cSimpleTypeDef ctermbg=darkyellow ctermfg=grey
	highlight cSimpleTypeDef ctermfg=42
	
	"-------------------------------------------------------"	
	"			DECLARATION
	"-------------------------------------------------------"	
	"call s:HL("cVarDeclar", s:olive, "", "")
	highlight cVarDeclar 	ctermfg=174
	"highlight cVarDeclar ctermbg=red 
	"call s:HL("cAllVars", s:olive, "", "")
	highlight cAllVars 	ctermfg=174
	"highlight cAllVars ctermbg=red 
	"s:navy	
	call s:HL("cStorageClass", s:pink, "", "")
	"--------------------------
	
	"highlight mycGotoCall ctermbg=Yellow ctermfg=black
	call s:HL("mycGotoCall", s:blue, "", "")
	

	"-------------------------------------------------------"	
	"			MACRO
	"-------------------------------------------------------"	
	highlight cInclude ctermfg=176
	"call s:HL("cInclude",s:accent, "", "")
	call s:HL("cDefine", s:blue, "", "")

	highlight mycDefine ctermbg=green ctermfg=black
	call s:HL("mycDefine", s:purple, "", "")

	call s:HL("cIncluded", s:accent, "", "")
	"highlight cIncluded ctermbg=red ctermfg=black
	
	"call s:HL("cIncludedLib",s:olive, "", "")
	highlight cIncludedLib ctermfg=180
	
	"call s:HL("cmacroName", s:highlight, "", s:bold)
	highlight cmacroName ctermbg=green ctermfg=black
	highlight cmacroNameNoArg ctermbg=green ctermfg=black
	
	"call s:HL("cmacroArgComma", s:highlight, "", "")
	"highlight cmacroArgComma ctermbg=red ctermfg=white

	"call s:HL("cmacroArg", s:olive, "", "")
	highlight cmacroArg ctermfg=174
	"highlight cmacroArg ctermbg=red ctermfg=white
	
	"highlight cmacroCall ctermbg=red
	call s:HL("cmacroCall", s:orange, "", "")

	"highlight cUpperCaseMacro ctermbg=yellow ctermfg=black
	call s:HL("cUpperCaseMacro", s:olive, "", "")
	
	"-------------------------------------------------------"	
	"			OPERATORS
	"-------------------------------------------------------"	
	highlight cOperator ctermfg=213
	highlight mycOperator ctermfg=220
	
	"highlight myArrowOperator ctermfg=220
	call s:HL("myArrowOperator", s:aqua, "", "")
	
	"highlight cSizeof ctermbg=red ctermfg=white
	highlight cSizeof ctermfg=42
	"highlight cCast ctermbg=darkmagenta ctermfg=white
	highlight cCast ctermfg=42
	call s:HL("semicolon", s:highlight, "", "")

	"-------------------------------------------------------"	
	"			FUNCTION
	"-------------------------------------------------------"	
	"highlight cFunction ctermfg=68 cterm=bold 
	highlight cFunctionPointer ctermbg=green ctermfg=black
	highlight cTypeFunc ctermbg=blue ctermfg=white

	"highlight cMacroFunctionCall ctermbg=red ctermfg=white
	highlight cMacroFunctionCall ctermfg=111
	highlight cMacroElseFunctionCall ctermfg=111
	"highlight cMacroFunctionCallSimpleMacro ctermfg=111
	highlight cMacroFunctionCallSimpleMacro ctermbg=red ctermfg=white
	
	highlight cMacroFunctionCallStr ctermbg=green ctermfg=black
	"highlight cMacroFunctionCallBis ctermbg=red ctermfg=white

	highlight cMacroFunctionCallBis ctermfg=111
	
	"call s:HL("cMacroFunctionCall", s:navy, "", "")
	"call s:HL("cMacroFunctionCallBis", s:accent, "", "")
	
	highlight cMacroUniqueFunc ctermbg=blue ctermfg=white
	"call s:HL("cMacroUniqueFunc", s:navy, "", "")

	highlight cFuncArgsType ctermfg=42
	"highlight cFuncArgsType ctermbg=red ctermfg=white
	
	"call s:HL("cFuncArgs", s:olive, "", "")
	highlight cFuncArgs ctermfg=174
	"highlight cFuncArgs ctermbg=42 ctermfg=white

	"highlight cFunction ctermbg=blue ctermfg=black
	highlight cFunction ctermfg=111
	"cterm=bold ctermfg=68
	"highlight cFunctionPointerArgs ctermbg=darkred ctermfg=blue
	highlight cFunctionPointerArgs ctermfg=42
	"highlight cFunctionPointerName ctermbg=grey ctermfg=white
	highlight cFunctionPointerName ctermfg=111
	"ctermfg=68 cterm=bold

	"highlight cFuncReturnFunc ctermbg=blue ctermfg=black
	highlight cFuncReturnFunc ctermfg=68 cterm=bold
	"highlight cFuncReturnFuncArgs ctermbg=yellow ctermfg=black
	"-------------------------------------------------------"	
	"			STRING
	"-------------------------------------------------------"	
	"call s:HL("mycStringChars", s:foreground, "", "")
	"call s:HL("cString", s:olive, "", "")
	"call s:HL("cCppString", s:olive, "", "")
	"highlight cString 	ctermfg=174
	call s:HL("cString", s:olive, "", "")
	"highlight cCppString 	ctermfg=174
	call s:HL("cCppString", s:olive, "", "")
	"highlight cCppString ctermbg=red ctermfg=white
	
	"highlight cFormat ctermfg=220
	"highlight cFormat ctermbg=red
	"call s:HL("cFormat", s:olive, "", s:bold)
	
	"-------------------------------------------------------"	
	"		   CONTAINED REGEXES
	"-------------------------------------------------------"	
	"call s:HL("cFuncAster", s:purple, "", "")
	highlight cFuncAster ctermfg=220

	call s:HL("cFuncArgsArray", s:highlight, "", "")
	
	call s:HL("opParen", s:highlight, "", "")
	call s:HL("cloParen", s:highlight, "", "")	
	call s:HL("opBracket", s:highlight, "", "")
	call s:HL("cloBracket", s:highlight, "", "")
	call s:HL("comma", s:highlight, "", "")
	call s:HL("column", s:highlight, "", "")
	highlight aTest ctermbg=darkyellow ctermfg=darkblue	
	"highlight comma ctermbg=red ctermfg=white

	call s:HL("cVaArg", s:olive, "", "")


	"avant 115
	"call s:HL("cConditional", s:orange, "", s:bold)
	highlight cConditional	ctermfg=213
	highlight cStatement	ctermfg=213
	highlight cRepeat	ctermfg=213
	...
	"	       	         LABELS
	"-------------------------------------------------------"	
	"highlight cLabel ctermfg=213
	"highlight mycLabel ctermbg=green
	call s:HL("mycLabel", s:blue, "", "")
	call s:HL("cLabel", s:blue, "", "")
	call s:HL("cUserLabel", s:blue, "", "")

	"JM uncommented this default == aqua
	call s:HL("cAnsiFunction",s:foreground, "", s:bold)
	"s:purple following default
	"call s:HL("cFunction",s:olive, "", "")
	"and this
	call s:HL("cAnsiName",s:pink, "", "")
	call s:HL("cDelimiter",s:blue, "", "")
	
	"JM following is green default s:pink in 2nd argument
	"foreground  | normal
	"s:comment   | purple
	"s:blue/aqua | bleu clair gras
	"s:purple    | yellow bold
	"s:pink      | green bold
	"s:navy      | bleu foncé
	"s:orange    | purple bold 
	...
	"else
	  "default foreground
	  "call s:HL("cFunction", s:orange, "", "")
	  "call s:HL("cCustomFunc", s:orange, "", "")
	  "call s:HL("cCustomFuncArgs", s:orange, "", "")
	"endif
